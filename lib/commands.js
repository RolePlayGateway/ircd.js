...
module.exports = {
  ...
  JOIN: function(user, channelNames, key) {
    ...
    if (!channelNames || !channelNames.length) {
      return user.send(this.host, irc.errors.needMoreParams, user.nick, ':Need more parameters');
    }
  ...
  PRIVMSG: function(user, target, message) {
    ...
    if (!target || target.length === 0) {
      ...
      if (!channel) {
        user.send(this.host, irc.errors.noSuchNick, user.nick, target, ':No such nick/channel');
  ...
  WHOIS: function(user, nickmask) {
    ...
    if (target) {
      ...
      user.send(this.host, irc.reply.whoIsIdle, user.nick, target.nick, target.idle, user.created, ':seconds idle, sign on time');
