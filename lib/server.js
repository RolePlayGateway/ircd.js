...
Server.prototype = {
    ...
    this.timeoutHandler = setInterval(function() {
      self.users.forEach(function(user) {
        if (user.hasTimedOut()) {
          ...
          user.send(self.host, 'PING', self.config.hostname, self.host);
        }
      });
    }, 10000);
  },
  ...
  start: function(callback) {
    ...
    server.channelList = [ '#lobby', '#debate', '#design', '#offtopic' ];
    server.channelList.map(function(channelName) {
      server.channels.registered[server.normalizeName(channelName)] = new Channel(channelName, server);
    });
    ...
    this.app.get('/channels', function(req, res){
      res.send(server.channelList);
    });
    
    this.app.post('/channels/:channelID', function(req, res){
      
      server.channelList.forEach(function(channelName) {
        console.log(req.body);
        var message = 'PRIVMSG ' + channelName + ' :' + req.body.message;
      
        server.channels.registered[channelName].send(message);
      });
