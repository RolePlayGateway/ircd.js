...
function Server() {
  ...
  this.bayeux = new faye.NodeAdapter({mount: '/live', timeout: 45});
...
Server.prototype = {
  ...
  serverCommands: {
    ...
    SERVER: function() {
      ...
      if (arguments.length < 6 && server.name === serverName) {
        ...
        this.completeLinkConnection(server, token);
  ...
  commands: {
    ...
    PRIVMSG: function(user, target, message) {
      ...
      if (!target || target.length === 0) {
        ...
        if (channel.isModerated && !user.isVoiced(channel)) {
          ...
          this.httpClient.publish('/channels/'+channel.name.substring(1)+'/messages', { message });
    ...
    WHO: function(user, target) {
      ...
      if (this.channelTarget(target)) {
        ...
        if (!channel) {
          user.send(this.host, irc.errors.noSuchChannel, user.nick, ':No such channel');
        } else {
  ...
  replyToLinkInitialization: function(server) {
    if (server.authenticated && server.gotServerInfo) {
      ...
      // Tell server about local connections
      var name = this.name;
      this.servers.each(function(s) {
        if (s.localConnection && s.name != server.name)
          server.send(':' + name, 'SERVER', s.name, s.hopCount, s.token, s.info);
      });

      // Tell other servers about the new connection
      //this.servers.broadcastOthers(server, ':' + name, 'SERVER', server.name, server.hopCount, server.token, server.info);
      ...
      for (var channelName in this.channels.registered) {
        var channel = this.channels.registered[channelName],
            users = channel.users.map(function(u) { return u.channelNick(channel); }).join(',');
        server.send('NJOIN', channel.name, users);
      }
    }
  },

  completeLinkConnection: function(server, token) {
    log('[' + this.name + '] Link complete: ' + server.name);
  },
  ...
  listenForLinks: function() {
    ...
    this.linkServer = net.createServer(function(stream) {
      ...
      try {
        ...
        carry.on('line',  function(line) {
          ...
          log('[' + server.name + ', L: ' + remoteServer.name + '] ' + line);
    ...
    this.linkServer.listen(this.config.linkPort);
  },
  ...
  start: function(callback) {
    ...
    this.httpClient = this.bayeux.getClient();
